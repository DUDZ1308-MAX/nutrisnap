...
function MiniWorkout({ workout, onEdit }: { workout: Workout; onEdit: () => void }) {
  const targets = workoutTargetAreas.filter((area) => workout.targetAreas?.includes(area.id));
  return <div className="group flex items-center gap-3 rounded-2xl bg-card/75 p-3.5" data-testid={`card-today-workout-${workout.id}`}><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-secondary text-primary"><Activity size={16} /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{workout.name}</p><p className="mt-0.5 text-xs text-muted-foreground">{workout.activity} · {workout.durationMinutes} min</p>{targets.length ? <p className="mt-1 truncate text-[10px] font-semibold text-primary" data-testid={`text-today-workout-targets-${workout.id}`}>{targets.map((target) => target.label).join(' · ')}</p> : null}</div><button type="button" onClick={onEdit} data-testid={`button-edit-today-workout-${workout.id}`} className="focus-ring grid size-8 place-items-center rounded-lg text-muted-foreground opacity-70 transition hover:bg-muted hover:text-foreground"><Pencil size={14} /></button></div>;
}

function MealRow({ meal, onEdit }: { meal: Meal; onEdit: () => void }) {
  return <div className="card-lift flex items-center gap-2.5 rounded-xl border border-border bg-background/55 p-2.5 sm:gap-3 sm:rounded-2xl sm:p-3.5" data-testid={`card-meal-${meal.id}`}><div className="grid size-9 shrink-0 place-items-center overflow-hidden rounded-lg bg-secondary text-primary sm:size-10 sm:rounded-xl">{meal.imageDataUrl ? <img src={meal.imageDataUrl} alt={`Photo of ${meal.name}`} className="h-full w-full object-cover" data-testid={`img-meal-${meal.id}`} /> : <Utensils size={14} />}</div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate text-xs font-bold sm:text-sm">{meal.name}</p><span className="hidden rounded-full bg-muted px-2 py-0.5 text-[9px] font-bold text-muted-foreground sm:inline">{meal.mealType}</span></div><p className="mt-0.5 text-[11px] text-muted-foreground sm:text-xs">{meal.time} · {meal.calories} kcal</p></div><button type="button" onClick={onEdit} data-testid={`button-edit-meal-${meal.id}`} className="focus-ring grid size-7 place-items-center rounded-lg text-muted-foreground transition hover:bg-muted hover:text-foreground sm:size-8"><Pencil size={13} /></button></div>;
}
...
function WorkoutCard({ workout, index, onEdit, onDelete }: { workout: Workout; index: number; onEdit: () => void; onDelete: () => void }) {
  const targets = workoutTargetAreas.filter((area) => workout.targetAreas?.includes(area.id));
  return <article className="card-lift stagger-in rounded-[22px] border border-border bg-card p-5 shadow-sm" style={{ animationDelay: `${Math.min(index * 60, 240)}ms` }} data-testid={`row-workout-${workout.id}`}>
    <div className="flex items-start justify-between gap-4"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-secondary text-primary"><Activity size={19} /></span><div><h3 className="font-display text-xl">{workout.name}</h3><p className="mt-0.5 text-xs text-muted-foreground">{workout.activity} · {monthDay(workout.date)}</p></div></div><div className="flex gap-1"><button type="button" onClick={onEdit} data-testid={`button-edit-workout-${workout.id}`} className="focus-ring grid size-8 place-items-center rounded-lg text-muted-foreground transition hover:bg-muted hover:text-foreground"><Pencil size={14} /></button><button type="button" onClick={onDelete} data-testid={`button-delete-workout-${workout.id}`} className="focus-ring grid size-8 place-items-center rounded-lg text-muted-foreground transition hover:bg-destructive/10 hover:text-destructive"><Trash2 size={14} /></button></div></div>
    <div className="mt-5 grid gap-3 rounded-2xl bg-muted/45 p-3 sm:grid-cols-[108px_minmax(0,1fr)] sm:items-center">
      <MuscleMap targets={workout.targetAreas ?? []} compact />
      <div className="min-w-0">
        <p className="font-mono-ui text-[9px] uppercase tracking-[.14em] text-muted-foreground">Target areas</p>
        {targets.length ? <div className="mt-2 flex flex-wrap gap-1.5" data-testid={`list-workout-targets-${workout.id}`}>{targets.map((target) => <span key={target.id} className="rounded-full bg-card px-2.5 py-1 text-[10px] font-bold text-foreground">{target.label}</span>)}</div> : <p className="mt-2 text-xs text-muted-foreground" data-testid={`text-workout-no-targets-${workout.id}`}>No areas selected for this session.</p>}
      </div>
    </div>
    <div className="mt-4 grid grid-cols-2 gap-3 border-t border-border pt-4"><div><p className="font-mono-ui text-[9px] uppercase tracking-[.14em] text-muted-foreground">Duration</p><p className="mt-1 text-sm font-bold">{workout.durationMinutes} min</p></div><div><p className="font-mono-ui text-[9px] uppercase tracking-[.14em] text-muted-foreground">Burned</p><p className="mt-1 text-sm font-bold">{workout.caloriesBurned} kcal</p></div></div>
    {workout.notes ? <p className="mt-4 rounded-xl bg-muted/60 px-3 py-2.5 text-xs leading-relaxed text-muted-foreground">“{workout.notes}”</p> : null}
  </article>;
}
